...
Status (2026-09-06): **Phase 0 complete; Phase 2 mostly built; Phase 3 drafted.** `resources.yaml` locks 31 artifacts (sha256 per architecture, cross-checked against upstream checksum files). Built natively on arm64 and passing `tests/smoke.sh` (non-root, no sudo, health, every agent CLI answers, no secrets in the environment): `mesa-tools`, `mesa-cli`, `mesa-jupyterlab`, `mesa-rstudio`, `mesa-vscode`, `mesa-agent-runner` and `mesa-kasm` (KasmVNC answers on 6901 with its basic-auth challenge; the DE proxy fronts it in production). amd64 builds and Harbor pushes wait for the AMD VM / CI secrets. The `k8s/` profiles are drafted and untested on a cluster. See `docs/plan.md` for the phases.
